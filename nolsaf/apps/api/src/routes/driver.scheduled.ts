  ...
  try {
    ...
    const [trips, total] = await Promise.all([
      prisma.transportBooking.findMany({
        ...
        include: {
          user: { select: { id: true, name: true, phone: true, email: true } },
              property: { select: { id: true, title: true, regionName: true, district: true, ward: true } },
          payout: { select: { id: true, status: true, grossAmount: true, commissionAmount: true, netPaid: true, currency: true } },
        },
    ...
    res.json({
      items: trips.map((trip) => ({
        ...
        payoutGrossAmount: trip.payout?.grossAmount ?? null,
        payoutCommissionAmount: trip.payout?.commissionAmount ?? null,
        payoutNetPaid: trip.payout?.netPaid ?? null,
