...
function getEnglishResponse(input: string): { response: string; type: ResponseType } {
  ...
  if (/^(hi+|hello|hey|hola|salut|good\s*(morning|afternoon|evening)|greetings|jambo|habari|mambo|niaje|sasa|shikamoo|hujambo|yo|sup|what'?s\s*up)/i.test(input)) {
    return {
      response: "Niaje! 👋 Naitwa Twiga 🦒, your friendly travel assistant at NoLSAF! How can I assist you today? 😊\n\nI can help with:\n🏨 Finding & booking verified properties\n🚗 Transport & driver services\n💳 Payments (M-Pesa, cards, bank transfer)\n👥 Group stays & custom trip planning\n🌍 Destinations across East Africa\n\nJust ask me anything!",
      type: "greeting"
    };
  }
  ...
  if (/(what is nolsaf|what'?s nolsaf|about nolsaf|nolsaf is|tell me about nolsaf|who is nolsaf|who are you|what do you do|what is this platform|explain nolsaf|nolsaf mean|meaning of nolsaf)/.test(input)) {
    return { response: "NoLSAF (No Limits, Stay Africa Free) is East Africa's trusted accommodation and travel platform 🌍\n\nWhat we offer:\n🏨 Verified property bookings — hotels, lodges, villas, apartments & more\n🚗 Integrated transport — airport pickups, city rides, inter-city travel\n💳 Flexible payments — M-Pesa, Tigo Pesa, Airtel Money, Visa/Mastercard\n👥 Group Stay — special rates for families, corporates & large groups\n🎯 Plan with Us — custom trip planning with local experts\n✅ Every property is verified before listing\n\nWe operate across Tanzania 🇹🇿, Kenya 🇰🇪, and expanding pan-Africa. What would you like to know more about?", type: "other" };
  }

  // === NOLSAF NAME / BRAND ===
  if (/(nolsaf stand|nolsaf mean|what does nolsaf|nolsaf full|nolsaf abbreviation|nolsaf acronym)/.test(input)) {
    return { response: "NoLSAF stands for \"No Limits, Stay Africa Free\" 🌍 — our vision is a platform where travelers explore Africa without barriers. Verified stays, seamless transport, and local payment options all in one place. The name reflects our pan-African ambition: starting in East Africa and expanding across the continent! 🚀", type: "other" };
  }
  ...
  if (/^(what|who|where|how|when|why|can|is|do|does|are|will|should|tell)/.test(input)) {
    return { response: "Great question! Here's a quick overview of NoLSAF 🌍\n\n🏨 Book verified properties across East Africa\n🚗 Arrange transport (airport, city, inter-city)\n💳 Pay with M-Pesa, cards, or bank transfer\n👥 Group Stay for families & large bookings\n🎯 Plan with Us for custom trips\n✅ All listings verified for trust & safety\n\nTry asking me something specific like:\n• \"How do I book a property?\"\n• \"Tell me about Zanzibar\"\n• \"What payment methods do you accept?\"\n• \"How do I register as an owner?\"\n• \"What's the best time to visit?\"\n\nI'm here to help! 😊", type: "other" };
  }
  ...
  // === DEFAULT RESPONSE ===
  return {
    response: "I'm not sure I caught that, but I'm here to help! 🤝\n\nHere's what I can assist with:\n🏨 Accommodation — search, book, manage\n🚗 Transport — rides, airport transfers\n💳 Payments — M-Pesa, cards, methods\n👥 Group Stay — families, corporates\n🎯 Plan with Us — custom trip planning\n🌍 Destinations — Tanzania, Kenya, Zanzibar…\n👤 Account — registration, login, referrals\n🏠 Owner & 🚗 Driver registration\n\nTry asking:\n→ \"How do I book a property?\"\n→ \"Tell me about Zanzibar\"\n→ \"What payment methods do you accept?\"\n→ \"How do I become a property owner?\"",
    type: "default"
  };
}
