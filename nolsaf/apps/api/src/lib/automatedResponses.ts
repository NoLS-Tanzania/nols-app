...
function getEnglishResponse(input: string): { response: string; type: ResponseType } {
  ...
  if (/(amenities|wifi|parking|pool|breakfast|air conditioning|facilities|features|what's included)/.test(input)) {
    return { response: "Each property on NoLSAF lists all amenities and features! 🎉 Common amenities include: 📶 WiFi, 🅿️ Parking, 🏊 Pool, 🍳 Breakfast, ❄️ Air conditioning, 🏋️ Gym, 🍽️ Restaurant, and more! Check individual property listings for complete details. Need a property with specific amenities? Use our filters 🔍 to search by what matters to you! Looking for something specific? Just let me know! 😊", type: "other" };
  }

  // === PROPERTY OWNERS ===
  if (/(property owner|owner|list property|become owner|rent property|host|landlord)/.test(input)) {
    return { response: "Property owners can join NoLSAF to list their accommodations 🏨 and reach travelers across East Africa! 🌍 Benefits include: ✅ Verified listings, 🔒 Secure payments, 👥 Professional support, and 📈 Access to a growing traveler base. To become an owner: Sign up as an owner 👤, verify your property ✅, complete your listing with photos 📸 and details 📝, and start receiving bookings! 💰 Our team helps with onboarding and property verification. Ready to grow your business? Let's get started! 🚀", type: "other" };
  }

  // === TRANSPORT / RIDES ===
  if (/(transport|ride|taxi|vehicle|car|bus|driver|pickup|drop off|travel|traveling|get to)/.test(input)) {
    return { response: "NoLSAF provides reliable transport services 🚗 across East Africa! Options include: 🚙 Private vehicles, 🚌 Shared transport, ✈️ Airport transfers, and 🗺️ Intercity travel. Our verified ✅ drivers offer safe 🔒, comfortable rides. Book transport through your account, specifying pickup/drop-off locations 📍 and travel dates 📅. All drivers are professional 👨‍✈️ and vehicles are regularly maintained 🔧. Need transport for your trip? We can arrange it! Just let me know your requirements! 😊", type: "other" };
  }
  ...
  if (/(confirmation|receipt|ticket|booking confirmation|voucher|booking number|reference)/.test(input)) {
    return { response: "After booking with NoLSAF, you'll receive: ✅ A booking confirmation email 📧 with your booking reference number, 📄 A digital receipt/ticket, and 📱 All details in your account dashboard. Keep your booking reference number handy! You'll need it for check-ins and any inquiries. Can't find your confirmation? Check your email or log into your account. Need help locating a booking? I'm here! 😊", type: "other" };
  }

  // === CHECK-IN / CHECK-OUT ===
  if (/(check in|checkout|check out|arrival|departure|early check in|late checkout|check in time)/.test(input)) {
    return { response: "Check-in and check-out details vary by property! 🏨 Standard times are usually: Check-in from 2-3 PM ⏰, Check-out before 11 AM-12 PM. Early check-in or late checkout may be available (subject to availability and sometimes extra fees). Contact your property directly or message them through your booking to arrange special times. Need specific information? Check your booking confirmation or property listing! 😊", type: "other" };
  }

  // === TRAVEL INSURANCE ===
  if (/(insurance|travel insurance|covered|protection|liability|medical insurance)/.test(input)) {
    return { response: "Travel insurance is always recommended for international trips! 🛡️ While NoLSAF doesn't provide travel insurance, we recommend purchasing it separately to cover: 🏥 Medical emergencies, ✈️ Trip cancellations, 🧳 Lost luggage, and other unforeseen circumstances. Check with your local insurance providers or travel agencies. Some credit cards also offer travel insurance benefits! Having insurance gives you peace of mind while traveling! 😊", type: "other" };
  }

  // === PACKING / WHAT TO BRING ===
  if (/(packing|what to bring|what to pack|clothing|luggage|suitcase|essentials|packing list)/.test(input)) {
    return { response: "What to pack depends on your destination and activities! 🎒 For safaris: Neutral colors (khaki, beige), comfortable shoes 👟, camera 📸, and binoculars. For beach destinations: Swimwear 🏖️, sunscreen 🧴, and light clothing. For mountain trekking: Warm layers, hiking boots ⛰️, and proper gear. Weather varies by season and location - check forecasts before packing! Need packing tips for a specific destination? Our agents can help! 😊", type: "other" };
  }
  ...
  if (/(currency|money|exchange rate|usd|dollar|shilling|kenyan shilling|tanzanian shilling|ugandan shilling|cash|atm|bank)/.test(input)) {
    return { response: "East Africa uses different currencies! 💵 Tanzania: Tanzanian Shilling (TZS), Kenya: Kenyan Shilling (KES), Uganda: Ugandan Shilling (UGX), Rwanda: Rwandan Franc (RWF). USD is often accepted in tourist areas. Exchange rates fluctuate - check current rates before travel. ATMs are available in major cities and towns 🏧. Credit cards accepted at hotels and larger establishments. It's good to have some local currency for small purchases and tips! Need currency advice? Let me know! 😊", type: "other" };
  }

  // === TIPPING / GRATUITY ===
  if (/(tip|tipping|gratuity|service charge|how much to tip|should i tip)/.test(input)) {
    return { response: "Tipping customs in East Africa: 💰 Tips are appreciated but not always mandatory. For guides and drivers: 10-15% or $5-20 per day depending on service. For restaurants: 5-10% if service charge isn't included. For hotel staff: Small amounts for bellboys, housekeeping. Always tip based on service quality and your budget. Local currency is preferred for tips. When in doubt, ask your guide or hotel staff about local customs! 😊", type: "other" };
  }

  // === PHOTOGRAPHY / CAMERAS ===
  if (/(camera|photography|photos|pictures|video|drone|permit|can i take photos)/.test(input)) {
    return { response: "Photography rules vary by location! 📸 In national parks and reserves: Cameras allowed, professional photography may need permits 📜. Drones: Usually require special permits - check regulations before bringing one 🚁. Some cultural sites: Photography may be restricted or require fees. Always ask permission before photographing people, especially in rural areas. Our agents can advise on photography permits for specific destinations. Capture your memories respectfully! 😊", type: "other" };
  }

  // === INTERNET / CONNECTIVITY ===
  if (/(internet|wifi|connection|data|mobile data|roaming|network|sim card|phone|connectivity)/.test(input)) {
    return { response: "Internet connectivity in East Africa: 📶 WiFi is available in most hotels, lodges, and restaurants in cities. Mobile data networks (3G/4G) are good in urban areas, coverage varies in remote locations. You can buy local SIM cards at airports or shops 📱. Major networks include: Safaricom, Airtel, Vodacom. Roaming charges apply with international SIMs. Some remote safari areas have limited connectivity - perfect for unplugging! Need connectivity tips? Let me know your destination! 😊", type: "other" };
  }

  // === FOOD / DINING / RESTAURANTS ===
  if (/(food|restaurant|dining|meal|cuisine|what to eat|local food|dietary|vegetarian|vegan|halal|kosher)/.test(input)) {
    return { response: "East African cuisine is diverse and delicious! 🍽️ Local favorites: Ugali with stew, nyama choma (grilled meat), pilau rice, chapati, and fresh tropical fruits. Restaurants range from local eateries to international cuisine. Most hotels offer breakfast, many include dinner options. Dietary restrictions? Many places accommodate vegetarian, vegan, halal, and other dietary needs - just mention when booking! Street food is popular but choose busy stalls. Ready to taste East Africa? 😊", type: "other" };
  }

  // === SHOPPING / SOUVENIRS / MARKETS ===
  if (/(shopping|market|souvenir|gift|buy|purchase|curio|handicraft|bargaining|haggle)/.test(input)) {
    return { response: "Shopping in East Africa is an adventure! 🛍️ Popular items: Handmade crafts, Maasai jewelry, wood carvings, textiles, coffee, tea, and spices. Local markets are great for authentic goods - practice friendly bargaining! 🗣️ Prices are often negotiable in markets. Fixed prices in shops and malls. Quality varies, so inspect items carefully. Our agents can recommend the best shopping spots and fair prices. Support local artisans and bring home unique memories! 😊", type: "other" };
  }

  // === SAFETY / SECURITY TIPS ===
  if (/(safe to travel|is it safe|safety tips|security advice|travel warning|travel advisory)/.test(input)) {
    return { response: "East Africa is generally safe for travelers! 🛡️ Safety tips: Stay aware of your surroundings, keep valuables secure, use hotel safes, avoid displaying expensive items, follow local advice, and respect local customs. Most tourist areas are well-policed. We recommend: Travel insurance, registering with your embassy, keeping copies of documents, and sharing itineraries with family. Our verified properties prioritize guest safety. For current advisories, check your government's travel website. Travel smart! 😊", type: "other" };
  }

  // === EMERGENCY / HELP ===
  if (/(emergency|urgent|help|stuck|problem|lost|stolen|accident|need help|immediate assistance)/.test(input)) {
    return { response: "For emergencies in East Africa: 🚨 Police: 999 (general), Medical: 911/112, Fire: 999. Contact your country's embassy for serious issues. NoLSAF support: Use your account's support section for booking-related emergencies 📞. Keep important numbers saved: Your hotel, transport provider, and travel agent. If something happens during your trip, stay calm and contact local authorities or your accommodation immediately. We're here to help with booking-related issues! Stay safe! 😊", type: "other" };
  }
  ...
  if (/(family|children|kids|baby|infant|child friendly|family friendly|age|children activities)/.test(input)) {
    return { response: "East Africa welcomes families! 👨‍👩‍👧‍👦 Family-friendly accommodations are available with amenities like family rooms, cots, and children's menus. Many safari lodges welcome children (age restrictions may apply for certain activities). Child-friendly activities: Wildlife viewing, beach activities, cultural visits, and educational experiences. Some properties offer babysitting services. When booking, mention children's ages so we can recommend suitable options. Family trips create amazing memories! Ready to plan your family adventure? 😊", type: "other" };
  }

  // === ACCESSIBILITY / SPECIAL NEEDS ===
  if (/(accessible|disability|wheelchair|mobility|special needs|accessible room|handicap)/.test(input)) {
    return { response: "NoLSAF is committed to accessible travel! ♿ Accessibility varies by property - check listings for accessible features. Some properties offer: Wheelchair accessible rooms, ramps, elevators, and accessible bathrooms. Safari vehicles may have limitations. Contact properties directly or mention your needs when booking so we can find suitable options. Our agents can help find accessible accommodations and activities. Everyone deserves great travel experiences! Need specific accessibility information? Let me know! 😊", type: "other" };
  }
