...
export default function SalesRateRequestsPage() {
  ...
  return <div className="w-full space-y-5 pb-10">
    ...
    <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(320px,400px)_minmax(0,1fr)]">
      <section className="min-w-0 self-start overflow-hidden rounded-lg bg-white ring-1 ring-neutral-200 xl:sticky xl:top-4">
      ...
      <section className="min-w-0 overflow-hidden rounded-lg bg-white ring-1 ring-neutral-200">
        ...
        {loading ? (
          ...
          <div>
            <div className="hidden grid-cols-[1.15fr_.8fr_.9fr_1.25fr_auto] gap-4 border-b border-neutral-200 bg-neutral-50 px-5 py-2.5 text-[10px] font-bold uppercase tracking-wider text-neutral-500 lg:grid sm:px-6">
              <span>Room and stay</span><span>Current rate</span><span>Proposed rate</span><span>Commercial reason</span><span>Status</span>
            </div>
            <div className="divide-y divide-neutral-100">
              {filteredRequests.map((request) => {
                const delta = request.currentRate ? ((request.proposedRate - request.currentRate) / request.currentRate) * 100 : 0;
                return <article key={request.id} className="grid min-w-0 gap-4 px-5 py-5 transition hover:bg-neutral-50/70 sm:px-6 lg:grid-cols-[1.15fr_.8fr_.9fr_1.25fr_auto] lg:items-center">
                  <div className="min-w-0"><p className="m-0 truncate text-sm font-bold text-neutral-950">{request.roomType.name}</p><p className="mb-0 mt-1 flex items-center gap-1.5 text-xs text-neutral-500"><CalendarDays className="h-3.5 w-3.5" />Stay on {formatDate(request.stayDate)}</p><p className="mb-0 mt-1 text-[10px] text-neutral-400">Submitted {formatDateTime(request.createdAt)}</p></div>
                  <div className="min-w-0"><p className="m-0 text-[10px] font-bold uppercase tracking-wider text-neutral-400 lg:hidden">Current rate</p><p className="mb-0 mt-1 truncate text-xs font-semibold text-neutral-500 line-through">{money(request.currentRate, request.currency)}</p></div>
                  <div className="min-w-0"><p className="m-0 text-[10px] font-bold uppercase tracking-wider text-neutral-400 lg:hidden">Proposed rate</p><p className="mb-0 mt-1 truncate text-sm font-bold text-emerald-800">{money(request.proposedRate, request.currency)}</p><span className={`mt-1 inline-block rounded px-1.5 py-0.5 text-[10px] font-bold ${delta >= 0 ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{delta >= 0 ? "+" : ""}{delta.toFixed(1)}%</span></div>
                  <div className="min-w-0"><p className="m-0 text-[10px] font-bold uppercase tracking-wider text-neutral-400 lg:hidden">Commercial reason</p><p className="mb-0 mt-1 line-clamp-2 text-xs leading-5 text-neutral-600" title={request.reason}>{request.reason}</p></div>
                  <div><p className="m-0 text-[10px] font-bold uppercase tracking-wider text-neutral-400 lg:hidden">Status</p><span className={`mt-1 inline-flex w-fit items-center gap-1.5 rounded-full px-2.5 py-1.5 text-[10px] font-bold ring-1 ${statusStyles[request.status] || statusStyles.DISMISSED}`}><StatusIcon status={request.status} />{request.status === "APPLIED" ? "APPROVED" : request.status === "DISMISSED" ? "DECLINED" : request.status}</span></div>
                </article>;
              })}
            </div>
          </div>
