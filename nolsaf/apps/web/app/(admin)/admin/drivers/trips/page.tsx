...
import { Calendar, Search, X, Truck, MapPin, Clock, DollarSign, BarChart3 } from "lucide-react";
    ...
    <div className="space-y-6 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
        <div className="flex flex-col items-center text-center">
          ...
          <p className="text-sm text-gray-500 mt-1">View and manage all driver trips and assignments</p>
        </div>
      </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {list.map((trip) => (
                    <tr key={trip.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{trip.tripCode}</td>
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="max-w-xs truncate">{trip.pickup}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-900">
                        <div className="flex items-center gap-2">
                          <MapPin className="h-4 w-4 text-gray-400" />
                          <span className="max-w-xs truncate">{trip.dropoff}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        <div className="flex items-center gap-2">
                          <Clock className="h-4 w-4 text-gray-400" />
                          <span>{new Date(trip.scheduledAt).toLocaleDateString()}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        <div className="flex items-center gap-2">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 text-xs font-medium rounded-full ${badgeClasses(trip.status)}`}>
                          {trip.status}
                        </span>
                      </td>
                    </tr>
              ...
              {list.map((trip) => (
                <div key={trip.id} className="border rounded-lg p-4 bg-white shadow-sm">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <div className="font-semibold text-gray-900">{trip.tripCode}</div>
                      <span className={`mt-1 inline-block px-2 py-1 text-xs font-medium rounded-full ${badgeClasses(trip.status)}`}>
                        {trip.status}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <div className="text-gray-900">{trip.dropoff}</div>
                      </div>
                    </div>
                    <div className="flex items-center justify-between pt-2 border-t border-gray-200">
                      <div className="flex items-center gap-2">
                        <Clock className="h-4 w-4 text-gray-400" />
                        <span className="text-sm text-gray-600">{new Date(trip.scheduledAt).toLocaleDateString()}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <DollarSign className="h-4 w-4 text-gray-400" />
