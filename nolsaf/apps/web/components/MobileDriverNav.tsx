...
export default function MobileDriverNav() {
  ...
  return (
    <nav aria-label="Driver mobile navigation" className="md:hidden fixed inset-x-0 bottom-4 z-50 px-4">
      <div className="mx-auto max-w-lg" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}>
        <div className="relative rounded-[2rem] border border-slate-200/90 bg-[linear-gradient(180deg,rgba(255,255,255,0.985),rgba(248,250,252,0.985))] px-3 pb-3 pt-5 shadow-[0_24px_60px_rgba(15,23,42,0.12)] ring-1 ring-slate-900/5 backdrop-blur-xl">
          <div className="pointer-events-none absolute inset-x-12 top-0 h-px bg-gradient-to-r from-transparent via-[#02665e]/12 to-transparent" aria-hidden />
          <div className="pointer-events-none absolute left-1/2 top-0 h-14 w-24 -translate-x-1/2 rounded-full bg-[#02665e]/5 blur-2xl" aria-hidden />

          <div className="grid h-[4.6rem] grid-cols-5 items-end">
            {renderNavItem({
              href: "/driver",
              slot: "home",
              label: "Home",
          ...
          <Link
            ...
            className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-[38%] no-underline"
            {...touch("claim")}
          >
            <span className="flex flex-col items-center" style={fabScale("claim")}>
              <span
                className="relative flex h-[3.35rem] w-[3.35rem] items-center justify-center rounded-full border border-white/25 shadow-[0_16px_34px_rgba(2,102,94,0.22)] ring-[5px] ring-white/88"
              ...
              <span className={`mt-2 text-[9px] font-medium uppercase leading-none tracking-[0.14em] ${active.claim ? "text-slate-700" : "text-slate-400"}`}>
                Claim
              </span>
              <span
                className={`mt-2 h-[3px] rounded-full transition-all duration-200 ${active.claim ? "w-8 bg-[#02665e] opacity-100" : "w-6 bg-slate-200 opacity-45"}`}
                aria-hidden
              />
            </span>
          </Link>
