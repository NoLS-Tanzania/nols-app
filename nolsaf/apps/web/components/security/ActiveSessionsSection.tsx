    ...
    <section className="rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-200 hover:shadow-md p-4 sm:p-6">
      <div className="flex flex-col sm:flex-row items-start gap-4 mb-6">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-semibold text-lg text-slate-900">Active Sessions</div>
          <p className="text-sm text-slate-600 mt-1">Review and manage devices currently signed into your account.</p>
        </div>
        <div className="shrink-0">
          <button
            ...
            className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg border border-slate-300 bg-white text-slate-700 font-semibold text-sm hover:bg-slate-50 hover:border-slate-400 active:scale-[0.98] transition-all duration-200 disabled:opacity-60"
            disabled={loading}
          >
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {sessions.map((s) => (
            <div key={String(s.id)} className="rounded-xl border border-slate-200 bg-white p-4 flex items-start justify-between gap-4">
              <div className="min-w-0">
                <div className="text-sm font-semibold text-slate-900 truncate">{s.userAgent || "Unknown device"}</div>
