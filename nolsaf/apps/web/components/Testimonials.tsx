  ...
  return (
    <section className="mt-8" aria-label={hideTitle ? "Testimonials" : undefined} aria-labelledby={hideTitle ? undefined : "testimonials-heading"}>
      <div className="public-container">
        ...
        {/* Mobile: horizontal snap-scroll carousel | sm+: grid */}
        <div className="sm:hidden flex gap-3 overflow-x-auto snap-x snap-mandatory pb-3 -mx-4 px-4"
        ...
        >
          {cards.map((t) => {
            ...
            const initials = (t.name || 'N').split(' ').map((p) => p.trim()[0]).filter(Boolean).slice(0, 2).join('').toUpperCase();

            return (
              <figure
                key={'m-' + t.name + t.title}
                className="snap-start flex-shrink-0 w-[72vw] max-w-[280px] flex flex-col rounded-2xl overflow-hidden bg-white border border-slate-100"
              ...
              >
                ...
                <div className="flex flex-col flex-1 px-4 pt-3 pb-3 gap-2.5" style={{ background: `radial-gradient(circle at 0% 0%, ${palette.shimmer} 0%, transparent 60%)` }}>
                  <div className="flex items-center gap-2">
                    <div className="flex-shrink-0 w-6 h-6 rounded-lg flex items-center justify-center" style={{ background: `${palette.bar}18`, border: `1px solid ${palette.bar}33` }}>
                  ...
                  <blockquote className="flex-1 text-[12px] leading-relaxed text-slate-600">&ldquo;{t.snippet}&rdquo;</blockquote>
                  <figcaption className="flex items-center justify-between gap-2 pt-2 border-t border-slate-100">
                    <div className="flex items-center gap-2 min-w-0">
                      <div className="w-6 h-6 rounded-full flex-shrink-0 flex items-center justify-center text-[10px] font-black text-white" style={{ background: `linear-gradient(135deg,${palette.avatarFrom},${palette.avatarTo})` }}>{initials}</div>
                      <div className="min-w-0 flex items-center gap-1.5">
                        <div className="text-[12px] font-semibold text-slate-900 truncate">{t.name}</div>
                        <div className="text-[10px] capitalize font-medium rounded-full px-1.5 py-px flex-shrink-0" style={{ background: palette.roleBg, color: palette.roleText, border: `1px solid ${palette.roleBorder}` }}>{t.roleType}</div>
                      </div>
                    </div>
                    <a href={href} target="_blank" rel="noopener noreferrer" aria-label={`View on ${platformLabel}`} className="flex-shrink-0 flex items-center gap-1 text-[10px] text-slate-400 hover:text-slate-600 transition-colors">
        ...
        <div className="sm:hidden flex items-center justify-center gap-1.5 mt-3">
          {cards.map((t, i) => (
            <span key={i} className="w-1.5 h-1.5 rounded-full bg-slate-300" aria-hidden />
          ))}
        </div>
