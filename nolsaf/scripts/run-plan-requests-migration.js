// Script to run the plan_requests migration
import 'dotenv/config';
import { readFileSync } from 'fs';
import { fileURLToPath } from 'url';
import { dirname, join } from 'path';
import { PrismaClient } from '@prisma/client';

const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);

const prisma = new PrismaClient();

async function runMigration() {
  try {
    const sqlPath = join(__dirname, '../prisma/migrations/add_plan_requests.sql');
    const sql = readFileSync(sqlPath, 'utf-8');
    
    // Remove comments and split by semicolons
    const cleanedSql = sql
      .split('\n')
      .map(line => line.trim())
      .filter(line => line && !line.startsWith('--'))
      .join('\n');
    
    console.log('Executing plan_requests migration...');
    
    // Execute the SQL
    try {
      await prisma.$executeRawUnsafe(cleanedSql);
      console.log('✓ Migration executed successfully');
    } catch (err) {
      // Check for specific error types
      const errorMsg = err.message || '';
      if (errorMsg.includes('already exists') || errorMsg.includes('Duplicate')) {
        console.log('⚠ Table already exists - migration may have already been applied');
        console.log('   Verifying table structure...');
        
        // Try to verify the table exists
        const tableCheck = await prisma.$queryRawUnsafe(`
          SELECT COUNT(*) as count 
          FROM information_schema.tables 
          WHERE table_schema = DATABASE()
          AND table_name = 'plan_requests'
        `);
        console.log('   Table exists:', tableCheck);
      } else {
        throw err;
      }
    }
    
    console.log('✅ Migration completed successfully!');
  } catch (error) {
    console.error('❌ Migration failed:', error);
    process.exit(1);
  } finally {
    await prisma.$disconnect();
  }
}

runMigration();
